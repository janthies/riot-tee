...
int main(void)
{
   /* Flash is separated into 32 * 32KB regions.
      Secure regions are defined by FIRST_NS_FLASH_REGION, all others are NS */
   for (uint32_t i = FIRST_NS_FLASH_REGION; i < 32; i++)
   {
   ...
   /* RAM is separated into 32 * 8 KB regions.
      Secure regions are defined by FIRST_NS_RAM_REGION, all others are NS */
   for (uint32_t i = FIRST_NS_RAM_REGION; i < 32; ++i)
   {
      /* Initial value is 0x0001 0101
         NS value is 0x0000 0101 */
   ...
   /* We configure the end of the last secure flash region as NSC
      NSC region is 32B, which is the smallest possible size */
   NRF_SPU_S->FLASHNSC[flash_nsc_id].REGION = NSC_FLASH_REGION;
   ...
   /* Configure CLOCK/POWER for NS access (LFCLK for ztimer/RTT) */
   NRF_SPU_S->PERIPHID[NRFX_PERIPHERAL_ID_GET(NRF_CLOCK_NS)].PERM &= ~(SPU_FLASHREGION_PERM_SECATTR_Msk);

   /* Configure RTC0 and RTC1 for NS access (ztimer_periph_rtt) */
